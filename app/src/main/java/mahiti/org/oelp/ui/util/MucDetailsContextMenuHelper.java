package mahiti.org.oelp.ui.util;
...
import mahiti.org.oelp.ui.ConferenceDetailsActivity;
import mahiti.org.oelp.ui.ConversationFragment;
import mahiti.org.oelp.ui.ConversationsActivity;
import mahiti.org.oelp.ui.MucUsersActivity;
...
public final class MucDetailsContextMenuHelper {
        ...
        if (user != null && user.getRealJid() != null) {
            ...
            if ((self.getAffiliation().ranks(MucOptions.Affiliation.ADMIN) && self.getAffiliation().outranks(user.getAffiliation())) || self.getAffiliation() == MucOptions.Affiliation.OWNER) {
                if (advancedMode) {
                    ...
                    if (!mahiti.org.oelp.Config.DISABLE_BAN) {
                        managePermissionsVisible = true;
    ...
    public static boolean onContextItemSelected(MenuItem item, MucOptions.User user, XmppActivity activity, final String fingerprint) {
    ...
    private static void kickFromRoom(final MucOptions.User user, XmppActivity activity, XmppConnectionService.OnAffiliationChanged onAffiliationChanged) {
        final Conversation conversation = user.getConversation();
    ...
    private static void banFromRoom(final MucOptions.User user, XmppActivity activity, XmppConnectionService.OnAffiliationChanged onAffiliationChanged) {
        final Conversation conversation = user.getConversation();
        AlertDialog.Builder builder = new AlertDialog.Builder(activity);
