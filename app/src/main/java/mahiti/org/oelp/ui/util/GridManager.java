...
public class GridManager {
    ...
    public static void setupLayoutManager(final Context context, RecyclerView recyclerView, @DimenRes int desiredSize) {
        ...
        Log.d(mahiti.org.oelp.Config.LOGTAG, "preliminary count=" + columnInfo.count);
        ...
        recyclerView.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                if (availableWidth == 0) {
                    Log.e(mahiti.org.oelp.Config.LOGTAG, "GridManager: available width was 0; probably because layout was hidden");
                    return;
                }
                final ColumnInfo columnInfo = calculateColumnCount(context, recyclerView.getMeasuredWidth(), desiredSize);
                Log.d(mahiti.org.oelp.Config.LOGTAG, "final count " + columnInfo.count);
                final RecyclerView.Adapter adapter = recyclerView.getAdapter();
                if (adapter != null && adapter.getItemCount() != 0) {
                    Log.e(mahiti.org.oelp.Config.LOGTAG, "adapter already has items; just go with it now");
                    return;
                }
