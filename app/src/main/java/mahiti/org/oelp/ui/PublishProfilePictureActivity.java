...
public class PublishProfilePictureActivity extends XmppActivity implements XmppConnectionService.OnAccountUpdate, OnAvatarPublication {
    ...
    private void chooseAvatar() {
        CropImage.activity()
                ...
                .setAspectRatio(1, 1)
                .setMinCropResultSize(mahiti.org.oelp.Config.AVATAR_SIZE, mahiti.org.oelp.Config.AVATAR_SIZE)
                .start(this);
    }
    ...
    protected void loadImageIntoPreview(Uri uri) {
        ...
        Bitmap bm = null;
        if (uri == null) {
            bm = avatarService().get(account, getPixel(mahiti.org.oelp.Config.AVATAR_SIZE));
        } else {
            try {
                bm = xmppConnectionService.getFileBackend().cropCenterSquare(uri, getPixel(mahiti.org.oelp.Config.AVATAR_SIZE));
            } catch (Exception e) {
