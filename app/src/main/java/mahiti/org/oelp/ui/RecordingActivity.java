    ...
    protected void stopRecording(final boolean saveFile) {
        ...
        if (!saveFile && mOutputFile != null) {
            if (mOutputFile.delete()) {
                Log.d(mahiti.org.oelp.Config.LOGTAG, "deleted canceled recording");
            }
        }
        if (saveFile) {
            new Thread(() -> {
                try {
                    if (!outputFileWrittenLatch.await(2, TimeUnit.SECONDS)) {
                        Log.d(mahiti.org.oelp.Config.LOGTAG, "time out waiting for output file to be written");
                    }
                } catch (InterruptedException e) {
                    Log.d(mahiti.org.oelp.Config.LOGTAG, "interrupted while waiting for output file to be written", e);
                }
                runOnUiThread(() -> {
        ...
        if (!noMedia.exists()) {
            try {
                if (noMedia.createNewFile()) {
                    Log.d(mahiti.org.oelp.Config.LOGTAG, "created nomedia file in " + parentDirectory.getAbsolutePath());
                }
            } catch (IOException e) {
