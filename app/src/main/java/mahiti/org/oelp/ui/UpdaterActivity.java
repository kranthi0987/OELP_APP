...
public class UpdaterActivity extends XmppActivity {
        ...
        if (getIntent() != null && getIntent().getStringExtra("update").equals("PixArtMessenger_UpdateService")) {
            ...
            if (dir.isDirectory()) {
                ...
                for (String aChildren : children) {
                    Log.d(mahiti.org.oelp.Config.LOGTAG, "AppUpdater: delete old update files " + aChildren + " in " + dir);
            ...
            builder.setMessage(getString(R.string.install_update))
                    .setPositiveButton(R.string.update, (dialog, id) -> {
                        Log.d(mahiti.org.oelp.Config.LOGTAG, "AppUpdater: downloading " + FileName + " from " + appURI);
                        //ask for permissions on devices >= SDK 23
                        if (isStoragePermissionGranted() && isNetworkAvailable(getApplicationContext())) {
                            //start downloading the file using the download manager
                            if (store != null && store.equalsIgnoreCase(XmppConnectionService.PlayStore)) {
                            ...
                            } else if (store != null && store.equalsIgnoreCase(XmppConnectionService.FDroid)) {
                            ...
                            Log.d(mahiti.org.oelp.Config.LOGTAG, "AppUpdater: failed - has storage permissions " + isStoragePermissionGranted() + " and internet " + isNetworkAvailable(getApplicationContext()));
                        }
                    })
                    .setNeutralButton(R.string.changelog, (dialog, id) -> {
                        Uri uri = Uri.parse(mahiti.org.oelp.Config.CHANGELOG_URL); // missing 'http://' will cause crash
                        try {
    ...
    private class DownloadTask extends AsyncTask<String, Integer, String> {
        ...
        protected String doInBackground(String... sUrl) {
            ...
            try {
                Log.d(mahiti.org.oelp.Config.LOGTAG, "AppUpdater: save file to " + file.toString());
                Log.d(mahiti.org.oelp.Config.LOGTAG, "AppUpdater: download update from url: " + sUrl[0] + " to file name: " + file.toString());
                ...
                if (parentDirectory.mkdirs()) {
                    Log.d(mahiti.org.oelp.Config.LOGTAG, "created " + parentDirectory.getAbsolutePath());
                }
        ...
        protected void onPostExecute(String result) {
            ...
            if (result != null) {
                ...
                Log.d(mahiti.org.oelp.Config.LOGTAG, "AppUpdater: failed with " + result);
                UpdaterActivity.this.finish();
            } else {
