        ...
        if (type == 0) {
            if (forGroup) {
                /*query = DBConstants.SELECT + DBConstants.ALL_FROM + DBConstants.MEDIA_CONTENT_TABLE +
                        DBConstants.WHERE + DBConstants.GROUP_UUID + DBConstants.EQUAL_TO +
                        DBConstants.SINGLE_QUOTES + groupUUID + DBConstants.SINGLE_QUOTES +DBConstants.OR+
                        DBConstants.IS_GLOBAL_SHARED+DBConstants.EQUAL_TO+1+ DBConstants.AND +
                        DBConstants.DELETE_MEDIA + DBConstants.NOT_EQUAL_TO + 1 +DBConstants.AND+
                        DBConstants.ACTIVE+DBConstants.EQUAL_TO+2+ DBConstants.ORDER_BY +
